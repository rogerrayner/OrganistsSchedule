...
public class ExportService(IMapper mapper) : IExportService
{
    public byte[] ExportHolyServicesToExcel(IEnumerable<HolyServiceDto> holyServices)
    {
        ...
        worksheet.Cells[1, 4].Value = "Reunião de Jovens?";
        ...
        foreach (var hs in holyServices)
        {
            ...
            worksheet.Cells[row, 3].Value = hs.Organist.ShortName;
            worksheet.Cells[row, 4].Value = hs.IsYouthMeeting ? "x" : string.Empty;
            row++;
        }
        ...
        worksheet.Cells[2, 4, row - 1, 4].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
